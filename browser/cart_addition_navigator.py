...
class CartAdditionNavigator:
    def __init__(self, model: str | None = None):
        ...
        resolved = llm_model("cart_addition_navigator") if model is None else model
    ...
    async def decide_next_action(
        ...
        try:
            ...
            logger.log_dl_state(
                f"cart_nav/{target_event}/step{step}/pre-llm",
                page.url,
                _pre,
            ...
            logger.debug(f"[CartNavigator] pre-llm DL 스냅샷 실패: {e}")
        dl_block = ""
        if dl_ctx:
            dl_block = (
...
"""
    ...
    async def run_for_event(
        ...
        for step in range(1, self._max_steps + 1):
            ...
            if action == "captured":
                try:
                    ...
                    logger.log_dl_state(
                        f"cart_nav/{target_event}/llm-captured",
                        page.url,
                        _s,
                        target_event=target_event,
                        extra={"step": step},
            ...
            if action == "impossible":
                try:
                    ...
                    logger.log_dl_state(
                        f"cart_nav/{target_event}/impossible/pre",
                        page.url,
                        _dl_pre,
                        target_event=target_event,
                    ...
                    logger.log_dl_state(
                        f"cart_nav/{target_event}/impossible/post-3s",
                        page.url,
                        _dl_post,
                        target_event=target_event,
                        extra={
                    ...
                    logger.log_dl_raw_peek(
                        f"cart_nav/{target_event}/impossible/post-3s-raw",
                        page.url,
                        _raw,
                        target_event=target_event,
                        extra={"step": step},
                    ...
                    logger.debug(f"[CartNavigator] impossible DL 로그 실패: {_e}")
            ...
            events = await get_captured_events(
                page, log_tag=f"cart_nav/{target_event}/step{step}/after-wait"
            )
            ...
            try:
                ...
                logger.log_dl_state(
                    f"cart_nav/{target_event}/step{step}/after-action",
                    page.url,
                    _snap,
                    target_event=target_event,
                    extra={
            ...
            try:
                ...
                logger.log_dl_raw_peek(
                    f"cart_nav/{target_event}/step{step}/miss-raw",
                    page.url,
                    _raw_m,
                    target_event=target_event,
        ...
        try:
            _dl_pre = await snapshot_datalayer_names(page)
            logger.log_dl_state(
                f"cart_nav/{target_event}/max_steps/pre",
                page.url,
                _dl_pre,
                target_event=target_event,
            ...
            _emerged = sorted(_post_sig - _pre_sig)
            logger.log_dl_state(
                f"cart_nav/{target_event}/max_steps/post-3s",
                page.url,
                _dl_post,
                target_event=target_event,
                extra={
            ...
            logger.log_dl_raw_peek(
                f"cart_nav/{target_event}/max_steps/post-3s-raw",
                page.url,
                _raw_end,
                target_event=target_event,
                extra={"step": self._max_steps},
            ...
            logger.debug(f"[CartNavigator] max_steps DL 로그 실패: {_e}")
