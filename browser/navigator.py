...
EVENT_CAPTURE_GUIDE: dict[str, str] = {
    "view_item": (
        "목표: 상품 상세 페이지(PDP) 진입 시 자동 발화. "
        "현재 홈/목록 페이지라면 상품 이미지나 상품명을 클릭해 PDP로 이동하세요. "
    ...
    "add_to_cart": (
        "목표: 장바구니 담기 버튼 클릭 후 add_to_cart 이벤트 발화. "
        "다음 순서로 판단하세요: "
        "(1) PDP가 아니면 먼저 상품 클릭 → PDP 이동. "
        "(2) PDP에 있으면 장바구니 버튼을 바로 클릭 시도. "
        "    카페24 패턴: button[onclick*='Basket'], a[onclick*='Basket'], "
        "    button[id*='buy'], #buy_now_btn, .EC-purchase-btn. "
        "(3) 히스토리에서 장바구니 버튼 클릭이 성공했으나 이벤트가 미발화됐다면 → "
        "    옵션 선택이 필요한 것. 스냅샷에서 미선택 상태인 사이즈/색상/옵션을 찾아 클릭한 뒤 "
        "    다음 스텝에서 장바구니 버튼을 다시 클릭하세요. "
        "    드롭다운·라디오·옵션 버튼 모두 클릭으로 처리 가능."
    ),
    "add_to_wishlist": (
        "목표: 찜하기/위시리스트 버튼 클릭 후 이벤트 발화. "
        "♡ 하트 아이콘, '찜', '찜하기', '관심상품', '좋아요' 텍스트, "
        "button[class*='wish'], button[class*='like'], button[class*='heart'], "
        "[class*='favorite'], [class*='bookmark'] 등. "
        "PDP 또는 PLP 상품 카드 위에 존재. 화면 밖이면 scroll 후 재시도."
    ),
    "view_item_list": (
        "목표: 카테고리/목록 페이지(PLP) 진입 시 자동 발화. "
        "현재 홈이라면 카테고리 링크를 찾아 navigate 액션으로 직접 이동. "
        "카테고리 URL 패턴: /product/list.html?cate_no=XXX, /category/XXX. "
        "클릭이 안 되는 메뉴 링크는 반드시 navigate를 사용하세요."
    ),
    "begin_checkout": (
        "목표: '구매하기', '바로구매', '결제하기' 버튼 클릭 후 이벤트 발화. "
        "장바구니에 상품이 없으면 먼저 add_to_cart를 수행해야 합니다. "
        "장바구니 페이지 또는 PDP에서 결제 버튼을 찾으세요."
    ),
    "view_cart": (
...
당신은 매 스텝마다 현재 페이지 스냅샷과 지금까지 실행한 액션 히스토리를 받습니다.
히스토리를 보고 "현재 어느 단계에 와 있는지" 파악한 뒤 다음 액션을 결정하세요.
...
- navigate: URL 직접 이동
- scroll: 페이지 스크롤 (버튼이 화면 밖에 있을 때)
- form_fill: 폼 입력 (더미 데이터만 사용)
- captured: 목표 이벤트가 이미 캡처됨
- impossible: 사이트 구조상 캡처가 근본적으로 불가능할 때만 사용

판단 규칙:
1. 히스토리에서 이미 완료된 단계를 확인하고, 다음 단계로 진행하세요.
2. 액션이 성공했으나 이벤트가 발화되지 않은 경우 → 추가 선행 조건이 있다는 신호입니다.
3. 목표 이벤트에 필요한 페이지가 아니면 먼저 이동하세요.
4. 버튼/링크가 화면 밖에 있을 수 있으니 scroll 후 재시도하세요.
5. 팝업, 레이어, 모달이 있으면 먼저 닫으세요.
6. impossible은 정말 사이트 구조상 캡처가 불가능할 때만 사용하세요.
    ...
    async def decide_next_action(
        ...
        if action_history:
            ...
            for h in action_history:
                outcome = "이벤트 발화됨" if h["event_fired"] else ("실패: " + h["error"] if h["error"] else "성공 but 이벤트 미발화")
                event_label = f"[{h['target_event']}] " if h.get("target_event") != target_event else ""
                lines.append(
                    f"  스텝{h['step']} {event_label}{h['action']} "
                    f"({h.get('selector') or h.get('url', '')}) → {outcome}"
                )
