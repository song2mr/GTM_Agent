...
EVENT_CAPTURE_GUIDE: dict[str, str] = {
    ...
    "select_item": (
        ...
        "PLP에서 상품 썸네일·제목 링크를 click. implicit의 ‘URL만 맞추기’와 다릅니다."
    ),
    "view_item_list": (
...
- **반드시 클릭·조작** 후에만 발화하는 이벤트입니다. navigate는 주로 PDP/장바구니 등 **실행할 화면으로 이동**할 때만 쓰고, 끝을 click으로 맞춥니다.
- `click`의 `selector`는 **단일** 유효 CSS만(쉼표로 나열 금지). 옵션이 여러 개면 **스텝마다 하나씩** 클릭합니다.
...
class LLMNavigator:
    ...
    async def decide_next_action(
        ...
        if sk == "interaction" and _url_looks_like_pdp(page.url):
            try:
...
(click 시 **CSS 셀렉터 하나만**. 쉼표로 여러 개 나열하지 마세요. 실행기는 첫 항목만 사용합니다.)
{history_text}
{budget_block}
페이지 HTML (축약):
{snapshot}
    ...
    async def run_for_event(
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            history_entry: dict = {
                ...
                "action": action,
                "selector": exec_decision.get("selector", "") if action == "click" else decision.get("selector", ""),
