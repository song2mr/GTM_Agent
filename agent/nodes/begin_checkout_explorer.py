"""Node 3.5: begin_checkout(결제 시작) 계열 전용 탐색.

Journey Planner가 `begin_checkout_events`에 넣은 이름만 처리합니다.
Active Explorer / Cart Addition 이후 `current_url`에서 이어서 별도 세션으로 시도합니다.
"""
...
async def begin_checkout_explorer(state: GTMAgentState) -> GTMAgentState:
    """결제 시작 계열 이벤트만 별도 브라우저 세션으로 캡처."""
    emit(
        "node_enter",
        node_id=3.5,
        node_key="begin_checkout_explorer",
        title="Begin Checkout Explorer",
    )
    update_state(current_node=3.5, nodes_status={"begin_checkout_explorer": "run"})
    _started = time.time()

    targets = list(state.get("begin_checkout_events") or [])
    if not targets:
        _dur = int((time.time() - _started) * 1000)
        emit("node_exit", node_id=3.5, status="skip", duration_ms=_dur)
        update_state(nodes_status={"begin_checkout_explorer": "skip"})
        return state
    ...
    logger.info(
        f"[BeginCheckoutExplorer] 시작 events={targets!r} resume_url={target_url!r} "
        f"headless={headless}"
    )
            ...
            for target_event in targets:
                ...
                if already:
                    exploration_log.append(
                        f"{target_event}: 이미 캡처됨 (begin_checkout 노드 스킵)"
                    )
                ...
                if target_event in click_triggers:
                    ...
                    if click_result.success:
                        ...
                        if dl_match:
                            ...
                            exploration_log.append(
                                f"{target_event}: 클릭 트리거 후 dataLayer 성공 (checkout 노드)"
                            )
                            event_capture_log.append(
                                {
                                    "event": target_event,
                                    ...
                                    "notes": "Begin Checkout 노드: 트리거 클릭 후 발화",
                                }
                            )
                            continue

                nav = BeginCheckoutNavigator()
                ...
                if result == "captured":
                    for e in await get_captured_events(page):
                        if e not in captured_events:
                            captured_events.append(e)
                    exploration_log.append(
                        f"{target_event}: BeginCheckoutNavigator 캡처 성공"
                    )
                    event_capture_log.append(
                        {
                            "event": target_event,
                            "method": "checkout_navigator_datalayer",
                            "result": "success",
                            "selector": "",
                            "notes": "결제 시작 전용 Navigator로 dataLayer 캡처",
                        }
                    )
                    continue
                ...
                if use_dom and dom_selectors:
                    ...
                    if dom_data:
                        ...
                        exploration_log.append(
                            f"{target_event}: Checkout Navigator 실패 → DOM 폴백"
                        )
                        event_capture_log.append(
                            {
                                "event": target_event,
                                ...
                                "result": "success",
                                "selector": "",
                                "notes": "Begin Checkout 노드에서 DOM 폴백",
                            }
                        )
                        continue
                ...
                exploration_log.append(f"{target_event}: checkout 노드 실패 → Manual")
                event_capture_log.append(
                    {
                        "event": target_event,
                        "method": "manual",
                        "result": "pending",
                        "selector": "",
                        "notes": "Begin Checkout 전용 절차 실패 → Manual",
                    }
                )
    ...
    _dur = int((time.time() - _started) * 1000)
    emit("node_exit", node_id=3.5, status="done", duration_ms=_dur)
    update_state(nodes_status={"begin_checkout_explorer": "done"})
