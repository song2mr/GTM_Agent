...
[사용자: 자연어 요청 + 대상 URL]
  ...
  Node 1: Page Classifier
    ...
    - GTM API로 기존 컨테이너 설정 조회
             ↓
  Node 2: Journey Planner (명시 목록 > 괄호 > LLM 폴백)
    - 이벤트 스코프 결정(`agent/request_events.resolve_selected_events`)
        1) state["selected_events"] (UI 체크박스)가 있으면 strict — 이 목록만 큐에 투입
        2) 없으면 `user_request`의 마지막 `( … )` 괄호 목록(CLI/레거시 호환)
        3) 둘 다 없을 때만 LLM이 큐를 제안(프롬프트는 표준 퍼널 자동 확장 금지)
    - 이벤트별 자동화 가능 여부 분류
    - 탐색 시퀀스 정렬(순서 중요): `_normalize_and_sort_exploration_queue`가
      `_EXPLORATION_RANK`로 **이미 큐에 있는 항목만** 정렬한다(새 항목 추가 안 함).
    ...
    - Manual Capture 필요 이벤트: 요청/선택에 purchase/refund가 있으면 `manual_required`에 추가(큐에는 미포함)
             ↓
...
class GTMAgentState(TypedDict):
    # 입력 (UI 폼 / CLI에서 전달)
    user_request: str          # 자유 텍스트 **메모** — 설치 대상을 직접 결정하지 않음
    target_url: str
    ...
    # 설치할 이벤트 **명시 목록** (UI 체크박스 / CLI `--events`)
    # 비어 있지 않으면 Journey Planner/Planning이 strict 모드로 이 목록만 처리.
    # 비어 있으면 user_request의 `( … )` 괄호 파서 → LLM 큐로 폴백.
    selected_events: list      # list[str], 소문자 GA4 이벤트명
...
## 13. 2026-04-19 문서 반영 현황 (CanPlan 재설계)

- `agent/canplan/` 모듈이 추가되어 `DraftPlan -> CanPlan` 정규화 경로를 구현했다.
- `gtm/spec_builder.py`를 통해 CanPlan에서 GTM API 스펙으로 직접 빌드하도록 경로를 분리했다.
- `STRICT_CANPLAN=1`에서 정규화 오류 시 레거시 보정 경로를 차단하도록 적용했다.
- `Journey Planner`는 `exploration_plan`(이벤트 + playbook) 필드를 함께 생성하도록 확장했다.
- 리포터는 `canplan_hash`, `normalize_errors` 요약을 출력해 HITL 표시와 API 전송 정합성을 추적한다.
